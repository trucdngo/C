...
// insert new character to list in Alphbetical order
void insert(listNodePtr *sptr, char value) {
    ...
    if (newPtr != NULL) {
        ...
        prevPtr = NULL;
        currPtr = *sptr;
        // search for the entry to insert the new node to the list
        while ((currPtr != NULL) && (value > currPtr->data)) {
            prevPtr = currPtr;
            currPtr = currPtr->nextptr;
        }
        // if after exhaust searching for entry point to insert the node
        // just to find out that there is no node entry point
        // insert node to beginning of the list
        // for example: insert A to B->C->D
        if (prevPtr == NULL) {
            newPtr->nextptr = *sptr;
            *sptr = newPtr;
            // else, insert node to the entry point
            // for example, insert C to A->B->D->E
        } else {
