...
int main (void) {
    ...
    if (pFile1 != NULL) {
        ...
        if (QueenPosition == -1) {
            ...
            while ((QueenPosition != -1) && (QueenPosition > 0)) {
                if (QueenPosition > 7) {
                    printf("%d is invalid Queen position.", QueenPosition);
                    printf("\nEnter -1 to exit\n");
                    printf("Place the first Queen row in any column[0 to 7]: ");
                    fprintf(pFile1, "%d is invalid Queen position.", QueenPosition);
                    fprintf(pFile1, "\nEnter -1 to exit\n");
                    fprintf(pFile1, "Place the first Queen row in any column[0 to 7]: ");
                    scanf("%d", &QueenPosition);
                    fprintf(pFile1, "%d", QueenPosition);
                }
                int selectedQpos = QueenPosition;
                int QueenMaxPos = 8;
                int resetCounter = 0;
                while (QueenPosition < QueenMaxPos) {
                    clearBoard();
                    queencol[++queencount] = QueenPosition;
                    colfree[QueenPosition] = FALSE;
                    upfree[queencount + QueenPosition] = FALSE;
                    ...
                    addQueen();
                    QueenPosition++;
                    if (QueenPosition == QueenMaxPos && resetCounter == 0) {
                        resetCounter++;
                        QueenPosition = 0;
                        QueenMaxPos = selectedQpos;
                    }
                } //end while
                printf("\nEnter -1 to exit\n");
                printf("Place the first Queen row in any column[0 to 7]: ");
